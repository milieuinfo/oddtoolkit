...
ODDToolkit\footnote{\url{https://github.com/milieuinfo/oddtoolkit}} is a standalone CLI that combines an OWL ontology~\cite{GRAU2008309}, a SKOS concept scheme~\cite{miles2009skos}, and project-specific settings capturing the architectural choices of the software development teams (e.g.,~handling primary keys and cardinality). The ontology remains the formal source model, the SKOS layer provides business or developer-facing names, and all artefacts are derived from one enriched intermediate representation.
